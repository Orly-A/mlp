...
def analyze_scores_choose_params(scores_file, hyper_param_list):
    ...
    """
    score_tab = scores_file.groupby(hyper_param_list)["mean_bal_acc_test"].mean().reset_index(). \
        sort_values(by=["mean_bal_acc_test"], ascending=False)
    print(score_tab.head(50))
    score_tab = scores_file.groupby(hyper_param_list)["mean_f1_score_test"].mean().reset_index(). \
...
if __name__ == "__main__":
    ...
    working_dir = "/vol/ek/Home/orlyl02/working_dir/oligopred/mlp/logs_first_tune_depth"
    # find all the log files in the working directory that have k5 in the name and end with .log
    log_files = [f for f in os.listdir(working_dir) if re.search(r'first.*.log', f) and f.endswith('.log')]
    ...
    scores_file.to_pickle(os.path.join(working_dir, "scores.pkl"))
    print(scores_file)
